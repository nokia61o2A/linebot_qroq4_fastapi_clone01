...
# [NEW] 回覆必附主選單 QuickReply + TTS（可切換）
# [CHANGED] LINE v3 SDK 呼叫改同步，不再 await
# [KEPT] 外匯查詢優先於美股代碼；人設切換（甜/鹹/萌/酷/random）
...
TTS_SEND_ALWAYS = os.getenv("TTS_SEND_ALWAYS", "true").lower() == "true"  # [NEW]
...
PERSONA_ALIAS = {"甜": "sweet", "鹹": "salty", "萌": "moe", "酷": "cool", "random": "random", "隨機切換": "random"}
...
def build_main_menu() -> FlexMessage:
    ...
    return attach_quick_reply(FlexMessage(alt_text="主選單", contents=bubble))
...
def build_submenu(kind: str) -> FlexMessage:
    ...
    return attach_quick_reply(FlexMessage(alt_text=title, contents=bubble))
...
def _is_fx_query(text: str) -> bool:
    t = text.strip().upper()
    if t in FX_CODES or t in FX_ALIAS.values(): return True
...
def render_fx_report(base: str, quote: str, link: str, last, chg, ts, df):
    trend = ""
...
def _is_stock_query(text: str) -> bool:
    ...
    if len(t) == 3 and t in FX_CODES: return False  # [CHANGED] 避免把幣別當美股
    if TW_TICKER_RE.match(t): return True
    if US_TICKER_RE.match(t) and t not in {"MENU"}: return True
    return False
...
def build_stock_prompt_block(stock_id: str, stock_name_hint: str) -> Tuple[str, dict]:
    debug_payload = {}
    ys = YahooStock(stock_id); debug_payload["yahoo_stock"] = {k: _safe_to_str(v) for k, v in vars(ys).items()}
    price_df = stock_price(stock_id); debug_payload["price"] = _safe_to_str(price_df)
    news = _remove_full_width_spaces(stock_news(stock_name_hint)); news = _truncate_text(news, 1024); debug_payload["news"] = _safe_to_str(news)
    fund_text = None; div_text = None
    ...
    debug_payload["fundamental"] = fund_text; debug_payload["dividend"] = div_text
    blk = []
    ...
    if stock_id not in ["^TWII", "^GSPC"]:
        ...
        blk.append(f"配息資料:\n{div_text}")
    blk.append(f"近期新聞資訊:\n{news}")
    return "\n".join(_safe_to_str(s) for s in blk), debug_payload
...
def render_stock_report(stock_id: str, stock_link: str, content_block: str) -> str:
    ...
    try:
        return get_analysis_reply([{"role":"system","content":sys},{"role":"user","content":content_block}])
    except Exception:
...
# [NEW] 單一出口：所有回覆都走這裡（附 QuickReply、可選附語音）
...
async def handle_text_message(event: MessageEvent):
    ...
    try:
        bot_info: BotInfoResponse = line_bot_api.get_bot_info()  # 同步
    ...
    if msg in ("大樂透", "威力彩", "539"):
        try:
            # 你原本的 get_lottery_analysis 在前版保留
            report = await run_in_threadpool(lambda: f"（示意）{msg} 趨勢＋三組號碼 ...")
...
async def handle_audio_message(event: MessageEvent):
    reply_tok = event.reply_token
    try:
        content_stream = line_bot_api.get_message_content(event.message.id)  # 同步
        ...
        line_bot_api.reply_message(
            ReplyMessageRequest(reply_token=reply_tok, messages=[attach_quick_reply(TextMessage(text=f"🎧 我聽到了：\n{text}"))])
        )
        ...
        if CLOUDINARY_URL:
            ...
            if audio_bytes:
                ...
                if url:
                    est = max(3000, min(30000, len(text) * 60))
                    line_bot_api.reply_message(
                        ReplyMessageRequest(reply_token=reply_tok, messages=[attach_quick_reply(AudioMessage(original_content_url=url, duration=est))])
                    )
...
async def lifespan(app: FastAPI):
    if BASE_URL:
        async with httpx.AsyncClient() as c:
            for endpoint in ("https://api-data.line.me/v2/bot/channel/webhook/endpoint", "https://api.line.me/v2/bot/channel/webhook/endpoint"):
                try:
