"""
aibot FastAPI 應用程序初始化 (v10 - 恢復 QuickReply 與 FlexMenu 的完整功能版)
"""
# ============================================
# 1. 匯入 (Imports)
# ============================================
...
# 🔥 核心修正 1: 恢復 Quick Reply 按鈕與 Flex 選單功能
...
def build_flex_menu(title: str, subtitle: str, actions: List[MessageAction]) -> FlexSendMessage:
    ...
    bubble = BubbleContainer(
        header=BoxComponent(layout="vertical", contents=[TextComponent(text=title, weight="bold", size="xl", color="#FFFFFF", align="center"), TextComponent(text=subtitle, size="sm", color="#EEEEEE", wrap=True, align="center", margin="md")], backgroundColor="#FF6B6B"),
        body=BoxComponent(layout="vertical", contents=buttons, spacing="sm", paddingAll="12px", backgroundColor="#FFF9F2")
    )
    return FlexSendMessage(alt_text=title, contents=bubble)

def flex_menu_finance(bot_name: str, is_group: bool) -> FlexSendMessage:
    prefix = f"@{bot_name} " if is_group else ""; actions = [MessageAction(label="📈 台股大盤", text=f"{prefix}大盤"), MessageAction(label="💰 金價查詢", text=f"{prefix}金價"), MessageAction(label="💴 日元匯率", text=f"{prefix}JPY")]
    return build_flex_menu("💰 金融服務", "快速查詢最新資訊", actions)

def flex_menu_lottery(bot_name: str, is_group: bool) -> FlexSendMessage:
    prefix = f"@{bot_name} " if is_group else ""; actions = [MessageAction(label="🎰 大樂透", text=f"{prefix}大樂透"), MessageAction(label="🎯 威力彩", text=f"{prefix}威力彩"), MessageAction(label="🔢 539", text=f"{prefix}539")]
    return build_flex_menu("🎰 彩票服務", "最新開獎資訊", actions)

def flex_menu_translate() -> FlexSendMessage:
    actions = [MessageAction(label="🇺🇸 翻英文", text="翻譯->英文"), MessageAction(label="🇹🇼 翻繁體中文", text="翻譯->繁體中文"), MessageAction(label="🇯🇵 翻日文", text="翻譯->日文"), MessageAction(label="🇰🇷 翻韓文", text="翻譯->韓文"), MessageAction(label="❌ 結束翻譯", text="翻譯->結束")]
    return build_flex_menu("🌐 翻譯選擇", "選擇目標語言", actions)

def flex_menu_persona() -> FlexSendMessage:
    actions = [MessageAction(label="🌸 甜美女友", text="甜"), MessageAction(label="😏 傲嬌女友", text="鹹"), MessageAction(label="🎲 隨機人設", text="random")]
    return build_flex_menu("💖 人設選擇", "切換 AI 女友的說話風格", actions)

def get_persona_info(user_id: str) -> str:
    p_key = user_persona.get(user_id, "sweet"); p = PERSONAS[p_key]
...
def build_persona_prompt(user_id: str, sentiment: str) -> str:
    ...
    return f"你是一位「{p['title']}」AI女友。你的角色特質是「{p['style']}」。根據使用者當前情緒「{sentiment}」，你應該「{emotion_tip}」。請用繁體中文、簡潔且帶有「{p['emoji']}」風格的表情符號來回應。"
...
def handle_message(event: MessageEvent):
    ...
    if reply_text is None:
        try:
            ...
            history.extend([{"role": "user", "content": msg}, {"role": "assistant", "content": reply_text}])
            conversation_history[chat_id] = history[-MAX_HISTORY_LEN*2:]
        except Exception as e:
            logger.error(f"AI 回覆失敗: {e}", exc_info=True); reply_text = "抱歉，我剛剛走神了 😅，可以再說一次嗎？"

    # --- 發送最終回覆 ---
    reply_simple(reply_token, reply_text, is_group, bot_name)
