...
# - 進入翻譯模式時，改用 sender.name：例「翻譯模式（中→英）」，
#   依 LINE 官方的 Icon/Nickname Switching： 
#   https://developers.line.biz/en/docs/messaging-api/icon-nickname-switch/
# - 更動機器人顯示名稱概念參考（每則訊息 sender，不改官方帳號主名稱）：
#   https://blog.jiatool.com/posts/line_change_display/
# =============================================================================
...
if not BASE_URL or not CHANNEL_TOKEN or not CHANNEL_SECRET:
    raise RuntimeError("請設定環境變數：BASE_URL、CHANNEL_ACCESS_TOKEN、CHANNEL_SECRET ；LINE 設定文件： https://developers.line.biz/en/docs/messaging-api/overview/")
...
try:
    ...
    if cloudinary.config().cloud_name:
        CLOUD_OK = True
        log.info("✅ Cloudinary 配置成功（上傳語音檔用）。參考：https://cloudinary.com/documentation/image_upload_api_reference")
except Exception as e:
    log.warning(f"⚠️ Cloudinary 初始化失敗：{e}（將改為純文字/無語音模式） 來源：https://cloudinary.com/documentation")
...
if OPENAI_API_KEY:
    try:
        ...
        if OPENAI_API_BASE:
            ...
            log.info(f"✅ OpenAI Client (base={OPENAI_API_BASE}) 參考：https://platform.openai.com/docs/api-reference")
        else:
            openai_client = openai_lib.OpenAI(api_key=OPENAI_API_KEY)
            log.info("✅ OpenAI Client (official) 參考：https://platform.openai.com/docs/api-reference")
    except Exception as e:
        log.warning(f"OpenAI 初始化失敗：{e} 參考：https://platform.openai.com/docs")
...
if GROQ_API_KEY:
    try:
        ...
        log.info("✅ Groq Client 初始化成功。API 文件：https://console.groq.com/docs")
    except Exception as e:
        log.warning(f"Groq 初始化失敗：{e}（請檢查 API Key） https://console.groq.com/docs")
# 僅使用可用的即時模型，避免 404
...
BOT_GOLD_URL = "https://rate.bot.com.tw/gold?Lang=zh-TW"  # 台銀金價頁： https://rate.bot.com.tw/gold?Lang=zh-TW
...
async def lifespan(app: FastAPI):
    ...
    try:
        async with httpx.AsyncClient() as c:
            ...
            r.raise_for_status()
            log.info("✅ Webhook 更新成功。設定介面說明：https://developers.line.biz/en/docs/messaging-api/building-bot/#setting-webhook-url")
    except Exception as e:
        log.warning(f"⚠️ Webhook 更新失敗：{e}（請檢查 BASE_URL 與 LINE Console） https://developers.line.biz/console/")
    yield
    log.info("👋 應用關閉")

app = FastAPI(lifespan=lifespan, title="LINE Bot", version="3.7.0")
router = APIRouter()
...
def display_sender_name(chat_id: str) -> Tuple[str, Optional[str]]:
    """
    利用 LINE sender 欄位改名字（僅針對該訊息的顯示，不改官方帳號主名稱）。
    官方文件（Icon/Nickname Switch）：https://developers.line.biz/en/docs/messaging-api/icon-nickname-switch/
    """
...
def reply_text_audio_flex(
    ...
    msgs = []
    # 1) Text
    text_msg = TextSendMessage(text=text, quick_reply=quick_bar())
    ...
    flex_msg = minimal_flex_hint(alt_text=(text[:60] + "…") if text else "提示", hint_text=hint_text)
...
def ai_chat(messages: List[dict]) -> str:
    """
    先嘗試 OpenAI（若有設定），失敗則使用 Groq；參考：https://console.groq.com/docs
    """
    if openai_client:
        try:
            ...
            log.warning(f"OpenAI 失敗：{e} ；API 參考：https://platform.openai.com/docs/api-reference/chat")
    ...
    try:
        ...
        log.warning(f"Groq 失敗：{e} ；文件：https://console.groq.com/docs")
        return "AI 引擎連線不穩定，請稍後再試。"

def translate_text(content: str, target_lang_display: str) -> str:
    """
    僅用 Groq（較省額度），指示只輸出譯文；參考：https://console.groq.com/docs
    """
    if not groq_client:
        return "抱歉，翻譯引擎暫不可用。"
    try:
...
def yahoo_snapshot(symbol: str) -> dict:
    ...
    try:
        ...
        log.warning(f"yfinance 快照失敗：{e} ；工具：https://pypi.org/project/yfinance/")
    return out
...
def get_bot_gold() -> Tuple[str, Optional[float], Optional[float], Optional[str]]:
    ...
    for u in urls:
        try:
            ...
            log.warning(f"抓取 {u} 失敗：{e} （台銀網站） https://rate.bot.com.tw")
...
def jpy_twd() -> str:
    try:
        ...
        if js.get("result") != "success":
            return "外匯 API 異常。 來源：https://www.exchangerate-api.com/docs/free"
        rate = js["rates"].get("TWD")
        if not rate:
            return "API 無 TWD 匯率。 來源：https://www.exchangerate-api.com"
...
def lottery_text(kind: str) -> str:
    try:
        ...
        return f"{kind} 官網讀取失敗。來源：https://www.taiwanlottery.com.tw/"
...
def tts_make_url(text: str, lang_code: str) -> Tuple[Optional[str], int]:
    ...
    gTTS 語言表參考：https://pypi.org/project/gTTS/
    """
    try:
        ...
        dur = max(1000, int(len(data)/32))  # 粗估，避免 0ms
    ...
    try:
        ...
        if text.startswith("翻譯->"):
            ...
            if lang in ("結束","結束翻譯"):
                ...
                # 這裡回覆會看到 sender.name 已變為「翻譯模式（中→英）」之類
        ...
        log.error(f"LINE 回覆失敗：{e}；錯誤碼說明：https://developers.line.biz/en/reference/messaging-api/#error-responses")
    except Exception as e:
