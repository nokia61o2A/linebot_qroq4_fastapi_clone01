...
def _extract_numbers_from_text(text: str) -> dict:
    """
    從大段純文字中盡量抓到「賣 / 買」價（元/公克）。全部用 get() 取值，絕不直接索引。
    """
    out = {}
    # 先抓賣
...
def _parse_gold_html(html: str) -> dict:
    ...
    # B) 嘗試 script JSON（有些版面會把資料塞在 script 變數）
    try:
...
def get_bot_gold() -> Tuple[str, Optional[float], Optional[float], Optional[str]]:
    """
    依序嘗試：
    1) rate.bot.com.tw/gold?Lang=zh-TW （桌機）
    2) rate.bot.com.tw/gold       （行動）
    3) 若仍缺，就回合併文本去抓；都抓不到，回預設訊息
    回傳：(訊息, 賣, 買, 掛牌時間)
    """
    urls = [
...
def lottery_text(kind: str) -> str:
    # 這裡保持簡潔，避免把不可控網站 selector 綁死；只要抓得到就讓 AI 說明
    try:
        if kind == "威力彩":
...
def on_message(event: MessageEvent):
    ...
    try:
        ...
        if low in ("金價","黃金","黃金價格"):
            ...
            reply_text_audio(event.reply_token, msg, audio, dur)
            return
        ...
        if text in ("開啟自動回答","關閉自動回答"):
            ...
            reply_text_audio(event.reply_token, f"自動回答：{'開啟' if auto_reply_status[chat_id] else '關閉'}")
            return
        ...
        if text.startswith("翻譯->"):
            ...
            if lang in ("結束","結束翻譯"):
                ...
                reply_text_audio(event.reply_token, "✅ 已結束翻譯模式")
            else:
                translation_states[chat_id] = lang
                reply_text_audio(event.reply_token, f"🈯 已開啟翻譯模式（→ {lang}）")
            return
        ...
        if chat_id in translation_states:
            ...
            reply_text_audio(event.reply_token, out, audio, dur)
            return
