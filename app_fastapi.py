...
# --- 繁體中文說明 ---
# 基礎設定，包含 Line Bot 與 Groq API 初始化
# ------------------------------------------ #
...
async def handle_message(event):
    ...
    if any(k in msg for k in ["威力彩","大樂透","539","雙贏彩"]):
        ...
        # --- 繁體中文說明 ---
        # 股票代號正則判斷（台股為純數字，美股為英文字母）
        # ------------------------------------------ #
        stock_code   = re.fullmatch(r"\d{4,6}[A-Za-z]?", msg)   # 2330 / 2317 / 2882A
        stockUS_code = re.fullmatch(r"[A-Za-z]{1,5}", msg)      # AAPL / TSLA / MSFT
        if stock_code:
